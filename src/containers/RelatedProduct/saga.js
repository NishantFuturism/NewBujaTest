...
import { ADD_TO_CART, ADD_TO_CART_SUCCESS, NEXT_PAGE, NEXT_PAGE_SUCCESS, SHOPPING_CART_DETAILS_RELETED } from './constants';
...
function* NextBtnAPI(action) {
  ...
  //const dealOfTheDayData = yield call(ProductDetailAPI.getRelatedproduct, action.pageNum, 10, action.SkuDetailId)
  //yield put({ type: NEXT_PAGE_SUCCESS, dealOfTheDayData });
  const shoppingcartDetails = yield call(mycartAPI.getShoppingcartDetails);
  yield put({ type: SHOPPING_CART_DETAILS_RELETED, shoppingcartDetails });
}
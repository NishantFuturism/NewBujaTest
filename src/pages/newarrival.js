...
export default function dealoftheday({topMenuData, footerPagesLinks, footerCategoryLinks,getNewArrivalData,isLoading, sellerDetailsFromDomain} ) {
  const addToCart = useSelector(state => state.addToCart)
  const [loading, setloading] = useState(isLoading ? isLoading: false);
  const [newArrival, setNewArrival] = useState(getNewArrivalData  || []);
  ...
  const changeFltr = (newFL) => {
    ...
    if (newFL) {
      if (router && router.pathname !== undefined) {
        ...
        CustomsAPI.newArrivalProduct(1, 100)
          .then(response => {
  ...
  return (
    <>
      <Head>
      <title>Shop New Arrival Products</title>
      <meta name="description" content={`Shop New Arrival Products`}></meta>
      </Head>
      <ToastContainer
        position="top-right"
      />
              ...
              <div className="product-shop-main-wrapper" id="product-listing">
                ...
                <div className="shop-product-wrap row " id="show">
                  ...
                  <input type="hidden" id="hdnMinPriceForReset" value="0" />
                    ...
                    {newArrival && !isFilterLoader && newArrival?.length === 0 ?

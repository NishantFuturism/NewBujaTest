...
import { 
  ...
  import { MavigationBar } from '../../containers/MavigationBar/mavigationBar';
...
export default function Header(props) {
  ...
  const [id, setId]=useState('mobileMenuContainer')
  ...
  useEffect(() => {
    ...
    CustomsAPI.getMultiMenu().then(response => {
      //setMenu(response)
  ...
  const quickviewcontainer = () => {
    ...
    <>
      {quiickviewdata?.SkuCode ? (
        <div className="modal fade show" role="dialog" id="QuickView" style={{ display: 'block', paddingLeft: '0px' }}>
                    ...
                    <div className="col-lg-7">
                      <div className="product-details-inner">
                        <div className="product-details-contentt">
                          ...
                          {quiickviewdata && quiickviewdata.ShortDescription !== null ? <div className="product-detail-sort-des pb-0 pt-0 product-quickView-desc">
                          ...
                          <div style={{ overflow: 'hidden', marginBottom: '5px' }}>
                            ...
                            <span style={{display:'flex'}}>                       
                              :
                              {/* &nbsp;{filtered.ListItem} */}
                              &nbsp;<div className="uom-box packsizeBox">
                                {/* {console.log('quiickviewdata',quiickviewdata.FilterList.length)} */}
                                  {quiickviewdata.FilterList?.length === 1 && (
                                    <span>{quiickviewdata.FilterList[0].ListItem}</span>
                                  )}
                                  {quiickviewdata.FilterList?.length > 1 && (
                                    <select className="selectpackSize" onChange={event => {
                                      changeFltr(event.target.value)
                                      // localStorage.setItem('selectedPacksize', event.target.value)
                                    }} value={Seleced} >
                                      {quiickviewdata && quiickviewdata.FilterList.map((itm) => (
                          ...
                          <AddToCart data={quiickviewdata} shoppingCart={shoppingCart} filtered={filtered} PDPpage style={{ marginRight: '-5px' }} isQuickView/>
    ...
    return (
        ...
        {
          isClient ? <>
      <Chatbot config={config} validator={validateInput} actionProvider={ActionProvider} messageParser={MessageParser} />
      <ChatbotLogo /></>
        : "" }
      ...
      <header className={`header-pos adibuja-header ${isSticky ? 'stykeyheader' : ''}`} key='header' >
                ...
                <div className="header-top-right d-flex align-items-center">
                  ...
                  <NavDropdown title={AccountName} id="header-nav-dropdown" >
                    ...
                    {!isLogin && (<>
                      ...
                      <div>
                        {/* <NavDropdown.Item onClick={(e) => {e.preventDefault(); router.push('/register')}}> */}
                        <Link href="/register" >
                        {' '}
        ...
        <MavigationBar isServerSide={isServerSide} sellerDetailsFromDomain={props.sellerDetailsFromDomain || sellerDetailsFromDomain }  CartData={CartData} wishlistdata={wishlistdata} menu={menu} multiMenu={props && props.topMenuData ? props.topMenuData : multiMenu} topMenuData={props && props.topMenuData ? props.topMenuData : multiMenu } />
        {/* <MavigationBar wishlistdata={wishlistdata} menu={menu} /> */}
        {popup === true ?
          <LocationPopup data={defaultlocdata} popup={popup} fetchToggle={fetchToggle} />
        : null}
        <div className="col-12 d-block d-lg-none" id={setId}>
          <div className="mobile-menu mean-container">
            <MavigationBar sellerDetailsFromDomain={props.sellerDetailsFromDomain || sellerDetailsFromDomain} CartData={CartData} id={id} wishlistdata={wishlistdata} menu={menu} multiMenu={props && props.topMenuData ? props.topMenuData : multiMenu} topMenuData={props && props.topMenuData ? props.topMenuData : multiMenu }/>
          </div>
        </div>
      </header >

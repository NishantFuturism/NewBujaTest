...
import BreadCrumb from '@/containers/MyAccount/savecartListBreadcrumb';
...
export function SavedCartListitem(props) {
  ...
  const deleteItem = async () => {
    if (props !== undefined) {
      SaveCartAPI.removesavecartlistitem(window.atob(localStorage.getItem('listname')), productSku)
        .then(response => {
  ...
  return (
    <>
      ...
      />
      <Header />
      <BreadCrumb myAccount="My Account" activepage="Saved Cart" />
      {/* <SubNavigation /> */}
      ...
      {isdeletepop && <DeletePopUp delete={deleteItem} viewCart={false} Fetchlist={fetchlist} closePopupModal={setIsDeletepop} />}
                    ...
                    <div className="row">
                      ...
                      <div className="col-12 col-sm-12 col-md-12 col-lg-10 saveListItem">
                              ...
                              <table id="tblSavedCart" className="table">
                                <thead>
                                  <tr style={{ whiteSpace: 'nowrap', overflow: 'hidden' }}>
                                    ...
                                    <th id="UnitPrice" width="100" style={{ textAlign: 'right' }}>Delivery Charges (₹)
                                      <button type='button' onClick={sortPrice} className="fa fa-sort"></button>
                                    </th>
                                ...
                                {loading === true ?
                                  <tbody>
                                    <tr className='rowSavecartdetail'>
                                      <td colSpan={7}>
                                        <div className='row'>
                                  ...
                                  <tbody>
                                    {(Sortdata || []).map(data =>
                                      <tr style={{ whiteSpace: 'nowrap', overflow: 'hidden' }} id="saveMobilerow" className={data.IsDeliveryAvailableToPinCode ? "pr-block item " : "pr-block item  delivery-not-available "}>
                                        ...
                                        <td><span style={{ display: 'none' }} className="moblbldsp">Pack Size : </span>{data.ListItem}</td>
                                        <td style={{ textAlign: 'right' }}><span style={{ display: 'none' }} className="moblbldsp">Price : </span>
                                          ₹{data.SPPrice}
                                        </td>
                                        <td style={{ textAlign: 'center' }}><span style={{ display: 'none' }} className="moblbldsp">Delivery Charges : </span>{data?.ShiprocketCharges != 0 ? `₹${data.ShiprocketCharges}` : <span style={{ color: 'green'}}>FREE Delivery</span>}</td>
                                        {data.IsDeliveryAvailableToPinCode ?
                                          <td style={{ textAlign: 'center' }}><span style={{ display: 'none' }} className="moblbldsp">Quantity : </span>{data.Quantity}</td> :
                                          <td style={{ textAlign: 'center' }} colSpan="1">
                                            Can't deliver to pin code <b> {locationPopupState && locationPopupState.pincode}</b>
                                          </td>}
                                        {data.SPPrice !== 0 ? <td style={{ textAlign: 'right' }}><span style={{ display: 'none' }} className="moblbldsp">Total : </span>₹{data.Total.toFixed(2)}</td> : <td style={{ textAlign: 'right' }}></td>}
                                ...
                                {savecartlistitem.length === 0 ?
                                  ...
                                  <tbody>
                                    <tr className="total-tr" >
                                      ...
                                      <td className='saveListtotal' colSpan="7" style={{ whitespace: 'nowrap', borderBottom: 'unset !important', right: "50px" }}>
                                        <b style={{ float: 'right', marginLeft: '-45px' }}>Total &nbsp;&nbsp;
                                          ₹{savecarttotal.toFixed(2)}</b>
                        ...
                        <div className="cart-button-wrapper d-flex justify-content-between mt-4">
                          <Link href="/" className="btn btn-secondary dark align-self-end cntechkt border border-dark">Continue Shopping</Link>
                          {console.log('313--', savecartlistitem.length)}
                          {
                            savecartlistitem.length > 0
                              ?
                              <Link className="btn btn-secondary dark align-self-end cntechkt border border-dark" id="logout" onClick={(e) => checkoutsavelist(e)} href="" >Continue Checkout</Link>
                              :
                              null
                          }
                        </div>

...
export function AddressList(props) {
  ...
  const [showAddNewAddressButton, setShowAddNewAddressButton] = useState(false)
  ...
  const [showFirstAddress, setShowFirstAddress] = useState(true)
  ...
  }, [datalist, showAddNewAddressButton, showAddressList]);
  ...
  function saveAddressForm() {
    fetchaddress()
    fetchAddressOfIndexPage() // this call is to check address list is empty or not before proceeding to buy
    cancelform()
  }
  ...
  useEffect(() => {
    if (customeraddress && customeraddress.length > 0) {
      ...
      if (showUpdateAddressForm) {
        setShowAddNewAddressButton(false)
      } else {
        setShowAddNewAddressButton(true)
      }
    }
  }, [customeraddress])
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if(primaryAddress?.City && primaryAddress?.ZipCode) {
      localStorage.setItem('pincodevalue',  primaryAddress.ZipCode);
      localStorage.setItem('sublocalityvalue', primaryAddress.City);
      Cookies.set('pincodevalue',  primaryAddress.ZipCode, { expires: 15 });
      Cookies.set('sublocalityvalue', primaryAddress.City, { expires: 15 });
  ...
  const slectaddress = (CustomerAddressId, data) => {
    ...
    if (data.CustomerAddressId === CustomerAddressId) {
      ...
      localStorage.setItem('CustomerAddressId', window.btoa(data.CustomerAddressId))
  ...
  function saveFirstAddressForm() {
    fetchaddress()
    fetchAddressOfIndexPage() // this call is to check address list is empty or not before proceeding to buy
    cancelform()
    setShowAddNewAddressButton(true)
  }
  const cancelFirstAddressForm = () => {
    setShowFirstAddress(false)
    setShowAddNewAddressButton(false)
    props.hideChooseDeliveryTimeSlot(false)
  }
  // add address first time
  const addNewaddressFirstTime = () => {
    setShowFirstAddress(true)
    setIsEdit(false);
    // this div is hide
    setShowAddNewAddressButton(false)
  ...
  // Edit form operattion
  // Edit form
  const editAddressForm = (todo) => {
    setIsEdit(true);''
  ...
  const saveEditAddressForm = () => {
    setShowAddNewAddressButton(true)
    setShowUpdateAddressForm(false)
    props.hideChooseDeliveryTimeSlot(false)
  }
  const cancelEditAddressForm = () => {
    setShowAddNewAddressButton(true)
    setShowUpdateAddressForm(false)
    props.hideChooseDeliveryTimeSlot(false)
    setIsEdit(false)
      ...
      .then(addressresponse => {
        ...
        return fetch(`${Constants.endPoints.geocode}key=${atosuggestdadta.GooglePlacesApiKey}&latlng=${responsew.result.geometry.location.lat},${responsew.result.geometry.location.lng}&sensor=${true}`, {
          ...
          .then(resaa => {
            ...
            //   Cookies.set('pincodevalue', pincode[0].long_name, { expires: 15 });
            // }
            ...
            if (city[0] && city[0].long_name && city[0].long_name !== "" && city[0].long_name === 'Pune' && address?.length > 0) {
              return fetch(`${Constants.endPoints.checkserviceavailability}setsession=true&address=${address[0].long_name}&area=${area[0].long_name}&city=${city[0].long_name}&lat=${responsew.result.geometry.location.lat}&lng=${responsew.result.geometry.location.lng}&pincode=${pincode[0].long_name}&placeid=${JSON.parse(resaa).results[0].place_id}&state=${state[0].long_name}`, {
                ...
                .then(response => {
                  ...
                  if (msg === "Available") {
                    ...
                    console.log("dsfs++_+",address);
                    if(address && address.length > 1){
                      setUpdateAddress(address[address.length - 1].long_name)
                      dispatch(getlocationtitle(address[address.length - 1].long_name, pincode[0].long_name, true))
                    }else{
                      setUpdateAddress(address[0].long_name)
  ...
  return (
    (addresslist?.length == 0 && loading === true) ?
      // change css
      <div className='row'>
          ...
          {(showFirstAddress === false) ?
            <p id="shippingnewaddressbtn">
              <button className="btn btn-default" type='button' id="shippingnewaddress"
                onClick={addNewaddressFirstTime}
            ...
            <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12 addr_form" style={{ display: 'block' }}>
                ...
                {detectlocview ?
                  ...
                  <div className="col-sm-6" style={{ paddingTop: '10px' }}>
                    <label id="lblDeliveryLocalityForCheckout" >
                      {(locationname || editdata.Address2) ? (locationname || editdata.Address2) : typeof window !== 'undefined' && localStorage.getItem('pincodevalue') ? `${localStorage.getItem('sublocalityvalue') || ''}, ${localStorage.getItem('pincodevalue')}` : 'Baner, Pune, Maharashtra, India, 411045'}&nbsp;{pin ? pin[0].long_name : editdata.ZipCode}
                    </label>
                    <div id="divDeliveryLocalitySuggestionsForCheckout" style={{ position: 'absolute', zIndex: 999, width: '280px' }}>
              ...
              {/* {alert('658')} */}
              {/* this form is open when zero address is there */}
              <AddressForm
                editdata1={editdata}
                atosuggestdadta={atosuggestdadta}
                userdetail={userdetail}
                // fetchaddress={fetchaddress}
                edit={false}
                Pin={pin}
                editdata={userdetail}
                AddressTypeName={customeraddress}
                Address={locationname || updateaddress}
                fetchAddressOfIndexPage={fetchAddressOfIndexPage}
                saveAddressForm={saveFirstAddressForm}
                cancelform={cancelFirstAddressForm}
              />
            </div>
          }
          {
            showAddNewAddressButton ?
              <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12 addr_form" style={{ display: 'block' }}>
                ...
                <AddressForm
                  ...
                  cancelform={cancelEditAddressForm}
                />
              </div>
              :
              null
                    ...
                    <div className="product-caption">
                      ...
                      <div className="mb-2">
                        ...
                        <button style={{ margin: '1px' }}
                          className="btn btn-success set-shipping-address btn-rfq checkout_btn default-address btn-fix fa fa-edit" type="button" onClick={() => editAddressForm(data)}>
                          Edit
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="product-item mt-2">
                      <div className="product-caption">
                        ...
                        <div className="mb-2">
                          ...
                          <button style={{ margin: '1px' }}
                            className="btn btn-secondary set-shipping-address btn-rfq checkout_btn default-address btn-fix fa fa-edit" type="button" onClick={() => editAddressForm(data)}>
                            Edit
                          </button>
                        </div>
                      </div>
          ...
          </div>
          {
            saveaddress ?
              <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12 addr_form" style={{ display: 'block' }}>
                ...
                {/* {alert('885')} */}
                {/* <AddressForm cancelform={cancelNewAddressForm} atosuggestdadta={atosuggestdadta} userdetail={userdetail} edit={false} setlist={setShowAddressList} Pin={pin} Address={updateaddress} /> */}
                {/* // this form is open when already atleast one address is present and click on add new address  */}
                <AddressForm
                  editdata1={editdata}
                  atosuggestdadta={atosuggestdadta}
                  userdetail={userdetail}
                  // fetchaddress={fetchaddress} 
                  edit={false}
                  // setlist={setShowAddressList}
                  Pin={pin}
                  editdata={userdetail}
                  AddressTypeName={customeraddress}
                  Address={locationname || updateaddress}
                  fetchAddressOfIndexPage={fetchAddressOfIndexPage} 
                  saveAddressForm={saveNewAddressForm}
                  cancelform={cancelNewAddressForm}
                />
              </div>
              : null
          }
          <br />
          {showAddressList && showAddNewAddressButton ?
            <p id="shippingnewaddressbtn">
          ...
          }
          {
            showUpdateAddressForm ?
              <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12 addr_form" style={{ display: 'block' }}>
                  ...
                  {detectlocview ?
                    ...
                    <div className="col-sm-8">
                      <label id="lblDeliveryLocalityForCheckout" >
                        ...
                        {isEdit ? editdata?.City : localStorage.getItem('sublocalityvalue') && localStorage.getItem('sublocalityvalue') !== "" ? localStorage.getItem('sublocalityvalue') : locationAPIData.DefaultDeliveryLocality_Area} {isEdit ? editdata?.ZipCode : localStorage.getItem('pincodevalue') && localStorage.getItem('pincodevalue') !== "" ? localStorage.getItem('pincodevalue') : locationAPIData.DefaultDeliveryLocality_Pincode}
                      </label>
                      <div id="divDeliveryLocalitySuggestionsForCheckout" style={{ position: 'absolute', zIndex: 999, width: '280px' }}>
                        <ul className="list-group" id="Suggestionlist"></ul>
                      </div>
                ...
                {/* // this form is open when click on edit button of particular address  */}
                <AddressForm
                  editdata1={editdata}
                  atosuggestdadta={atosuggestdadta}
                  userdetail={userdetail}
                  ...
                  Address={locationname || updateaddress}
                  fetchAddressOfIndexPage={fetchAddressOfIndexPage}
                  saveAddressForm={saveEditAddressForm}
                  cancelform={cancelEditAddressForm}
                />
              </div>
              : null

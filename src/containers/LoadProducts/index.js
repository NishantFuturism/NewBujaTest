...
import { addToComapre, addTowishlist, enablequickreviw, getcompare } from './actions';
...
export function LoadProducts(props) {
  ...
  const [filteredImageGallery, setFilteredImageGallery] = useState([]);
  ...
  const changeFltr = (newFL) => {
    ...
    if(filteredImageData?.length > 0) {
      ...
      setFilteredImageGallery([defaultImageData[0]])
    }
    setFiltered(P);
  ...
  //   setQtyCout(1)
  // }
  useEffect(() => {//console.log('heythereyouarehere1',props.shppingcart)
            ...
            {filtererror ? <h1>no data</h1> :
              <div key={props.index} className={props.skulisting ? "product-item subproduct_item mb-30 pr-block" : "product-item  mb-30 pr-block"} >
                <div className="product-thumb">
                  {isClient && (<Link href={`/product/${props.data.PageUrl}`} className="ajaxload" onClick={() => LoadProductPDP(props.data.PageUrl || props.data.SkuLink)}>
                    ...
                    {filteredImageGallery?.length && filteredImageGallery?.length > 1 ? (
                      <Carousel interval={null} indicators>
                        {filteredImageGallery.map((imgData, index) => (
                          <Carousel.Item interval={0} key={index}>
                            <Image 
                              alt={props.data.Name} 
                  ...
                  <div className="box-label">
                    ...
                    <div className="action-links">
                      <a onClick={() => AddToWishlist(filtered)} title="Add to Wishlist" ><i className="lnr lnr-heart" /></a>
                      <a onClick={() => AddToCompare1(props.data.SkuId)} title="Compare"><i className="lnr lnr-sync" /> </a>
                      <a to title="Quick view" className="LoadProduct'coffee-bean') quickmodalview" onClick={(event) => LoadProduct(event, props.data.PageUrl)} >
                        <i className="lnr lnr-magnifier" /*onClick={handleQuickLink}*/ />
                      </a>
                    </div>
                  </div>
              ...
              <i>{props.data.IsVeg}</i>
                    ...
                    <h4>
                      {
                        (props.data.DisplayName && props.data.DisplayName !== "") || (props.data.Name && props.data.Name !== "") ?
                          <Link href={`/product/${props.data.PageUrl}`} onClick={() => LoadProductPDP(props.data.PageUrl || props.data.SkuLink)} prefetch={false}>{props.data.DisplayName ? props.data.DisplayName : props.data.Name}</Link>
                          :
                          <Link href={`/product/${props.data.PageUrl}`} onClick={() => LoadProductPDP(props.data.PageUrl || props.data.SkuLink)} prefetch={false}>{props.data.name ? props.data.name : props.data.SkuCode}</Link>
                      }
                  ...
                  <div className="price-box">
                    <div>
                      ...
                      {filtered.FilterSPPrice !== filtered.FilterMRPPrice ?
                        <span>
                          {filtered &&
                  ...
                  <div className='twobox'>
                    <div className="uom-box">
                      {isClient && props.data.FiltersList.length === 1 && (
                      ...
                      {isClient && props.data.FiltersList.length > 1 && (
                        ...
                        }} value={Seleced} aria-label="State">
                          {props.data.FiltersList.map((itm, index) => (
                            <option key={index} value={itm.ListItem}>{itm.ListItem} - ₹{parseFloat(itm.FilterSPPrice).toFixed(2)}  {filtered.IsDiscountedSFP ? `${filtered.FilterDiscount} % off` : null}</option>
                          ))}
                        </select>
                      )}
                    </div>
            ...
            <div className="table">
              ...
              <div className="listviewinfo">
                  ...
                  <div className="product-thumb">
                    ...
                    <div className="box-label">
                      ...
                      <div className="action-links">
                        ...
                        <Link href="#" onClick={() => AddToCompare1(props.data.SkuId
                ...
                <div className='productlistdata'>
                  <div className='listviewProductname'>
                    <div className="product-name listname">
                      <h4>
                        <a href={`/product/${props.data.PageUrl}`} onClick={() => LoadProductPDP(props.data.PageUrl || props.data.SkuLink)}>{props.data.SkuCode ? props.data.SkuCode : props.data.DisplayName}</a>
                        {/* getting skucode for bag search display */}
                        {/* <Link href="/product">{ props.data.DisplayName}</Link> */}
                      </h4>
                    </div>
                  </div>
                  <div className='listviewRegularprice'>

...
export function RelatedProduct(props) {
  ...
  const [Dealofday, setgetDealofday] = useState(props.similarProductsData && props.similarProductsData.skuListingModels || []);
  ...
  const nextData = useState(props.similarProductsData.skuListingModels)
  const addToCart = useSelector(state => state.addToCart)
  const dispatch = useDispatch()
  useEffect(() => {
    if (props.similarProductsData && props.similarProductsData.skuListingModels) {
            ...
            <div className="col-12" >
              ...
              {Dealofday && Dealofday.length === 0 ? null :
                <Carousel breakPoints={responsive} itemsToShow={4} itemsToScroll={1} pagination={false} showEmptySlots>
                  {Dealofday !== undefined && (Dealofday.slice(sliceNum, sliceNum + Dealofday.length) || []).map((mapdata, i) => (
                    mapdata.FiltersList.length !== 0 ?
                      <div className="tab-content relatedProductBox" key={sliceNum}>
                        <div className="tab-pane fade show active" id="brand-one" style={{ display: 'block' }} >
                          <div className="product-gallary-wrapper">
                            <div className="product-gallary-active owl-carousel owl-arrow-style sale-nav owl-theme owl-loaded">
                              <div className="owl-stage-outer">
                                <div className="owl-stage"
                                  ...
                                  <div className=" active" >
                                    {/* <Product addToCartOBJ={addToCartOBJ} data={mapdata} index={i} /> */}
                                    <LoadProducts data={mapdata} shppingcart={shoppingcart} index={i} skulisting/>
                                  </div>
                                </div>
                              </div>
                            </div>

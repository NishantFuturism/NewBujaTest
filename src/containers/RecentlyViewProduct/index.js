...
export function RecentlyViewProduct() {
  ...
  useEffect(() => {
    if (FirstCall > 0 && Homedata !== undefined && Homedata.recentdataviewd !== undefined) {
      ...
      ProductDetailAPI.getrecentlyviewd(Cookies.get('recentlyViewed'), 1, 9)
      .then((resrecentlyAPI)=>{
  ...
  }, [Homedata, Cookies.get('recentlyViewed')])
  // useEffect(() => {
  ...
  const CustomNext = ({ onClick, ...rest }) =>
    // const { carouselState: { currentSlide } } = rest;
    <button type="button" className="arrow right" aria-label="rightbtn"
      onClick={() => {
        setsliceNum(sliceNum + initialWidth())
  ...
  return (
    <>
      {Dealofday !== undefined ?
        <div className="container-fluid viewcartreorder homescreenrecently" style={{display: Dealofday.length === 0 ? 'none' : 'block'} }>
          <div className="row">

...
const ReturnAndExchange = ({ returnAndExchangePolicyData, sellerIDFromServer, sellerDetailsFromDomain }) => {
  const [Ftr, setFooter] = useState(returnAndExchangePolicyData);
  ...
  return (
    <>
      <Head>
        <title>{returnAndExchangePolicyData?.PageTitle}</title>
        <meta name="description" content={returnAndExchangePolicyData?.MetaDescription} />
        <meta name="keywords" content={returnAndExchangePolicyData?.MetaKeyword}></meta>
        <meta property="og:title" content={returnAndExchangePolicyData?.PageTitle} />
        <meta property="og:url" content={`/${returnAndExchangePolicyData?.PageUrl}`} />
        <meta property="og:description" content={returnAndExchangePolicyData?.MetaDescription}/>
      </Head>
      ...
      <BreadCrumb activepage="Return & Exchange Policy"/>
      </div>
...
function getReturnAndExchangePolicyData(sellerIdMain) {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.footer}pagename=cancel-refund-and-exchange&cid=1&languageId=2&sellerid=${sellerIdMain}`;

...
import WalletAPI from '../../containers/MainPage/api/wallet';
...
export function Wallet() {
  ...
  const loadorderNumber = (OrderNumber) => {
    // console.log('OrderNumber11',OrderNumber)
     router.push(
      {
                                          ...
                                          <tbody>
                                            {(transactlist || []).map(data =>
                                              <tr style={{ whiteSpace: 'nowrap', overflow: 'hidden' }} className="pr-block item">
                                                ...
                                                <td className="text-center" id="orderwallet"><span className="d-lg-none">Order : </span><a href onClick={() => loadorderNumber(data.OrderNumber)} style={{ cursor: 'pointer', color: '#007bff' }}>{data.OrderNumber}</a></td>

...
export function ManageAddress(props) {
  ...
  const findAndSetAddress = (detectMyLocationData) => {
  ...
  function slectaddress(CustomerAddressId, ZipCode) {
    if (ZipCode !== ConstantsValues.pincode) {
  ...
  const editLocation = () => {
    ...
    console.log("SDFSD$$$",locationname);
  }
              ...
              return fetch(`${Constants.endPoints.checkserviceavailability}setsession=true&address=${address[0].long_name}&area=${area[0].long_name}&city=${city[0].long_name}&lat=${responsew.result.geometry.location.lat}&lng=${responsew.result.geometry.location.lng}&pincode=${pincode[0].long_name}&placeid=${JSON.parse(resaa).results[0].place_id}&state=${state[0].long_name}`, {
                ...
                .then(response => {
                  ...
                  if (msg === "Available") {
                    ...
                    Cookies.set('pincodevalue', pincode[0].long_name, { expires: 15 });
                    setPin(pincode)
                    if(address && address.length > 0 && address[0] && address[0].long_name){
                      setSearchAddress(address[0].long_name)
                    }
                    // else if(address && address.length > 0 && address[1] && address[1].long_name){
  ...
  const checkIfLocalityAndAdmLvl3Available = (cityData) => {
    if(cityData && cityData.length > 1){
      if(cityData[0].long_name === 'Pune' ||  cityData[1].long_name === 'Pune'){
        return true
      }
    }else if(cityData && cityData.length == 1){
      if(cityData[0].long_name === 'Pune'){
        return true
      }
    }
    return false
    ...
    const success = (position) => {
      fetch(`${Constants.endPoints.geocode}key=${atosuggestdadta.GooglePlacesApiKey}&latlng=${position.coords.latitude},${position.coords.longitude}&sensor=${true}`, {
        ...
        .then(addressresponse => {
          ...
          const address = findAndSetAddress(addressresponse)
          ...
          const area = findAndSetAddress(addressresponse)
          console.log(`manageaddressCity${JSON.stringify(city)}`);
          console.log("address.length > 0 && pincode.length > 0 && area.length > 0 && state.length > 0 && city[0] && city[0].long_name === 'Pune'",address , pincode, area, state, city);
          ...
          return fetch(`${Constants.endPoints.checkserviceavailability}setsession=true&address=${address[0].long_name}&area=${area[0].long_name}&city=${city[0].long_name}&lat=${position.coords.latitude}&lng=${position.coords.longitude}&pincode=${pincode[0].long_name}&placeid=${JSON.parse(addressresponse).results[0].place_id}&state=${state[0].long_name}`, {
            ...
            .then(response => {
              ...
              if (response == "Available") {
                ...
                if (address.length > 0 && pincode.length > 0 && area.length > 0 && state.length > 0 && checkIfLocalityAndAdmLvl3Available(city)) {
                  localStorage.setItem('sublocalityvalue', city[0].long_name)
                  localStorage.setItem('pincodevalue', pincode[0].long_name)
                  Cookies.set('sublocalityvalue', city[0].long_name, { expires: 15 });
                  Cookies.set('pincodevalue', pincode[0].long_name, { expires: 15 });
                ...
                setPin(pincode)
                console.log("address==>>>nis", address);
                setSearchAddress(city[0].long_name)
                        ...
                        <div className="tab-content dashboard-content" data-addressgroup="shipping" id="profile">
                          ...
                          <div className="">
                            <div id="address_list" className="row address-view">
                              ...
                              {!isEdit && <div className="col-lg-12 add-new-address" style={{ marginBottom: "20px" }}>
                                <div className="defaul_add">
                              ...
                              {enableaddressformn ?
                                <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12 addr_form" style={{ display: 'block' }}>
                                    ...
                                    {detectlocview ?
                                      ...
                                      <div className="col-sm-6">
                                        <label id="lblDeliveryLocalityForCheckout" >
                                          {isEdit ? editdata?.City : localStorage.getItem('sublocalityvalue') && localStorage.getItem('sublocalityvalue') !== "" ? localStorage.getItem('sublocalityvalue') : atosuggestdadta.DefaultDeliveryLocality_Area} {isEdit ? editdata?.ZipCode : localStorage.getItem('pincodevalue') && localStorage.getItem('pincodevalue') !== "" ? localStorage.getItem('pincodevalue') : atosuggestdadta.DefaultDeliveryLocality_Pincode}
                                  ...
                                  <AddressForm
                                    ...
                                    Pin={pin} 
                                    editdata={userdetail} 
                                    Address={locationname || searchaddress} 
                                    AddressTypeName={customeraddress} 
                                    fetchAddressOfIndexPage={fetchaddress} 
                                  />
                                </div>
                                : null
                              }

...
function shiprocketCreateOrder(Transactionid, paymentType, CustomerAddressId, paymentmethod){
  ...
  const orderPlacedForm = 'Reactweb'
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.shiprocketCreateOrder}`
  const databody = {
    
      "cartId": (localStorage.getItem('CartGUID') === null || localStorage.getItem('CartGUID') === undefined) ? ConstantsValues.defaultCartGUID : window.atob(localStorage.getItem('CartGUID')),
...
function updateDeliveryslotTime(OrderId, DeliverySlotDate, DeliverySlotID, InstructionForDeliverySlot) {
  ...
  // Logistics use
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.Updatedeliveryinfosendordersmsemail}currencyCode=${ConstantsValues.currencyCode}&custGUID=${(localStorage.getItem('CustGUID') === null || localStorage.getItem('CustGUID') === undefined) ? ConstantsValues.defaultCustGUID : window.atob(localStorage.getItem('CustGUID'))}&Instructionfordelivery=${InstructionForDeliverySlot}&clientId=${ConstantsValues.ClientId}&email=${EmailId}&phone=${UserMobile}&fullName=${firstName}${lastName}&OrderNumber=${OrderId}&sellerid=${localStorage.getItem('sellerID') === null || localStorage.getItem('sellerID') === undefined ? ConstantsValues.defaultSellerID : localStorage.getItem('sellerID')}`

          ...
          <div className="col-12">
            <div className="breadcrumb-wrap">
              <nav aria-label="breadcrumb" className='d-flex align-items-center'>
                <ul className="breadcrumb" data-bd={JSON.stringify(breadcrumb)}>
                  <li className="breadcrumb-item"><Link href="/">Home</Link></li>
                  {breadcrumb.filter(data => data.breadcrumb !== 'category').map((data, index) =>
                    <li key={index} className="breadcrumb-item active" aria-current="page"
                    // onClick={handleclik}
                    >
                      {/* <a href={`${index===0 ? "" : data.href}`} style={{ color: '#333131' }}> */}
                      <Link href={breadcrumb[2]?.href} style={{ color: '#333131' }}>
                      { 
                          index === 0 ?
                  ...
                  <li>
                    <span className='prodCount' style={{ marginLeft: "5px", fontSize: "16px", color: '#6c757d' }}>{`${`(${props.Productcount || 0})`}`}</span>
                  </li>
                </ul>
              </nav>
            </div>

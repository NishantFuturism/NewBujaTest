...
const AboutUs = ({ aboutUsData, sellerIDFromServer, sellerDetailsFromDomain }) => {
  const [Ftr, setFooter] = useState(aboutUsData);
  ...
  return (
    <>
      <Head>
        <title>{aboutUsData?.PageTitle}</title>
        <meta name="description" content={aboutUsData?.MetaDescription} />
        <meta name="keywords" content={aboutUsData?.MetaKeyword}></meta>
        <meta property="og:title" content={aboutUsData?.PageTitle} />
        <meta property="og:url" content={`/${aboutUsData?.PageUrl}`} />
        <meta property="og:description" content={aboutUsData?.MetaDescription}/>
      </Head>
...
function getAboutUsData(sellerIdMain) {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.footer}pagename=about-us&cid=1&languageId=2&sellerid=${sellerIdMain}`;

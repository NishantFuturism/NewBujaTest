...
import { loginWithPasswordAction, sendOtpPswd, userRegisterationUsingGmail, getCustIdByEmail,clearLoginWithPasswordAction } from '../../containers/Login/actions';
...
export function LoginwithPassword() {
  ...
  useEffect(() => {
    ...
    if (firstLoadCheck > 0 && sendOtpApiResponseData && sendOtpApiResponseData.sendOTPDataPswd !== undefined) {
      ...
      if (firstLoadCheck > 0 && sendOtpApiResponseData.sendOTPDataPswd.IsCustomerExist && sendOtpApiResponseData.customerlogindata !== undefined && sendOtpApiResponseData.customerlogindata.CustomerId === 0 && sendOtpApiResponseData.customerlogindata.GUID === '00000000-0000-0000-0000-000000000000') {
        ...
        dispatch(clearLoginWithPasswordAction())
      }
      if (firstLoadCheck > 0 && sendOtpApiResponseData.sendOTPDataPswd.IsCustomerExist && sendOtpApiResponseData.customerlogindata !== undefined && sendOtpApiResponseData.customerlogindata.CustomerId > 0) {
        setShowMsgTopBar(false)
        setisError(false)
        // setMsg('Redirect to Login Page..')
        // dispatch(DirectMessage('Redirect to Login Page..'))
        ...
        if (homeReducerData && homeReducerData.allCartData.cart) {
          mycartAPI.updatecart(null, homeReducerData.allCartData.cart[0].cart[0].Quantity, homeReducerData.allCartData.cart[0].cart[0].SkuId, homeReducerData.allCartData.cart[0].cart[0].SKUFilterPriceId, homeReducerData?.couponcode);
        }
      }
      if (firstLoadCheck > 0 && sendOtpApiResponseData.sendOTPDataPswd.IsCustomerExist === false && sendOtpApiResponseData.customerlogindata !== undefined && sendOtpApiResponseData.customerlogindata.CustomerId === 0) {
        setShowMsgTopBar(true)
        setisError(true)
        // setMsg('Redirect to Registration Page..')
        setMsg('Please enter correct mobile number (10 digit) or Email Id')
        // setTimeout(() => {
        //   router.push({ pathname: '/register', state: { textvalue, redrectFrom: '/login-form' } })
        //   setShowMsgTopBar(false)
        // }, 3000)
        setTimeout(() => {
          setShowMsgTopBar(false)
  ...
  return (
    <>
      ...
      // autoClose={50000}
      />
      {typeof window !== 'undefined' && window.localStorage ? showMsgTopBar && < Success isError={isError} msg={msg} close={closeMsgTopBar} /> : null }
                ...
                <div className="section-title text-center">
                  <p align="center" style={{ padding: '10px,0px' }}>
                    <Link href="/">
                      ...
                      {isClient ? <img alt='brand-logo' src={`${logoURL ? logoURL : '/assets1/img/icon/adibuja-logo.svg'}`} width="198" height="42"  style={{ paddingLeft: '23px' }} /> : ''}
                    </Link>
                  </p>
                </div>
                        ...
                        <div className='login-form'>
                          <div id="divloginwithusername" >
                              ...
                              <div className="form-group row align-items-center mb-3">
                                ...
                                <div className="col-sm-6">
                                  ...
                                  <button className="logwithpass" type="button" style={{ color: 'gray', position: 'absolute', top: '9px', right: '25px' }}>
                                    ...
                                    <i id="pass" aria-hidden role="button" onClick={() => setShown1password(!Shown1password)} onKeyDown={!Shown1password} className={`fa ${Shown1password ? 'fa-eye' : 'fa-eye-slash'}`}>
                                    </i>
                                  </button>
                                  <p>
                                    <Link href="/forgotpassword" style={{ position: 'absolute', top: '45px', right: '15px', color: '#CA251A' }}><u>Forgot Password?</u></Link>
                                  </p>
                                </div>
                              </div>
                            ...
                            <div className="text-center pt-20 pb-20 top-bordered">
                              <p><b>New To Adibuja? <Link href="/register" style={{ color: 'gray' }}><u>Create an account</u></Link></b></p>
                            </div>

...
import reducer from '../../../../../../containers/Trackorder/reducer';
import saga from '../../../../../../containers/Trackorder/saga';
...
//   console.log('dd', window.atob(localStorage.getItem('OrderStatus')));
  useEffect(() => {
    ...
    const formboday = {
      "OrderNumber": window.atob(localStorage.getItem('OrderNumber')),
      "ClientID": 1,
      "OrderGuid": dataOrder && dataOrder[0] && dataOrder[0].OrderGUID ? dataOrder[0].OrderGUID :  ConstantsValues.OrderGuid,
      "CustGuid" : window.atob(localStorage.getItem('CustGUID'))
    }
  ...
  const handlestatusclick = (id) => {
    ...
    // if (props && props.OrderItemDetails !== undefined) {
    dataOrder = JSON.parse(dataOrder)
    let OrderGuid = dataOrder && dataOrder[0] && dataOrder[0].OrderGUID ? dataOrder[0].OrderGUID :  ConstantsValues.OrderGuid;
    let CustGuid = window.atob(localStorage.getItem('CustGUID'))

    TrackOrderAPI.trackorederstatusDetailsItem(id,OrderGuid,CustGuid)
      .then(resp => {
        console.log('uuuuudetails', resp)
        // setorderstatusitemdetails1(resp)    
        setorderstatusitemdetails(resp)
      ...
      <>
      {
        ...
        (isUserLogin === false)
          ...
          <div className='Track-order'>
            ...
            />
            <Header />
            <BreadCrumb myAccount="My Account" activepage="Track Order" />
            {/* <SubNavigation URL={URL} Productcount={0} FliterFlag={false} ResetFlag={false} parentcategoryName={'myorders'} productcategoryName={'trackorder'} /> */}
          </div>
      }
      <div className="container-fluid">
        <h4><b>Order Number : {typeof window !== 'undefined' ? window.atob(localStorage.getItem('OrderNumber')) : ""}</b></h4>
        {/* <div>
        ...
        {trackOrderStatus.map((item, index) => {
          ...
          return <div key={index} className='trackorder'>
        ...
        <Accordion defaultActiveKey="0" flush style={{ width: '100%' }} >
          {orderstatusitem && orderstatusitem.map((itm,btnIndex) => <button key={btnIndex} type='button' style={{ fontSize: '15px', fontStyle: 'bold' }} onClick={() => { handlestatusclick(itm.OrderStatusId, itm.OrderNumber) }}> + {itm.OrderStatusName}</button>)}
          <>
            {handleClick === true &&
              <>
                <Accordion.Item eventKey="0">
                  <Accordion.Body>
                    <table className="table" id='trackordertable' style={{ width: '100%' }}>
                      ...
                      <tbody>
                        {orderstatusitemdetails && orderstatusitemdetails.map((item,orderIndex) =>
                          <tr key={orderIndex}>
                            <td className="alignleft" style={{ textAlign: 'left' }}>
                              {console.log("item.ProductImage",item)}
                              {/* <img src={} style={{ width: '60px', height: '60px' }} /> */}
                              <Image src={item.ProductImage} width="60" height="60" alt={item.ProductName} objectPosition={'center'} />
                            </td>
                            <td className="alignleft">
                              {item.ProductName
                              }
        ...
        <div className="cart-button-wrapper d-flex justify-content-between mt-4" id='trackorderbackbtn' style={{ paddingBottom: '20px' }
        }> <Link href={`/account/myorders/myorder-detail/${typeof window !== 'undefined' ? window.atob(localStorage.getItem('OrderNumber')) : ""}`} className="btn btn-secondary order-btn">← Back to My Order </Link> </div >
      </div >
      <Footer />
    </>
    );

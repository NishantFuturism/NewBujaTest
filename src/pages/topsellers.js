...
export default function dealoftheday({topMenuData, footerPagesLinks, footerCategoryLinks,getTopsellersData,isLoading, sellerDetailsFromDomain} ) {
  const addToCart = useSelector(state => state.addToCart)
  const [loading, setloading] = useState(isLoading ? isLoading: false);
  const [TopsellersProduct, setTopSellersProduct] = useState(getTopsellersData  || []);
  ...
  const changeFltr = (newFL) => {
    ...
    if (newFL) {
      if (router && router.pathname !== undefined) {
        ...
        CustomsAPI.getTopsellingdb(1, 100)
          .then(response => {
  ...
  return (
    <>
      <Head>
      <title>Shop Top Sellers Products</title>
      <meta name="description" content={`Shop Top Sellers Products`}></meta>
      </Head>
      <ToastContainer
        position="top-right"
      />

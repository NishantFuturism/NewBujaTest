  ...
  const submit = () => {
    ...
    fetch(`${Constants.urls.baseUrl}${Constants.endPoints.token}`, {
      ...
      .then((result) => {
       ...
       //dispatch(newPasswordapi(spttoken, usernameValue, n1passwordTxt, cnfpasswordTxt, true))
       CustomsAPI.NewPasswordApi(spttoken, usernameValue, n1passwordTxt, cnfpasswordTxt, true)
       .then((setpass)=>{
          // if (ressetpass !== undefined && ressetpass !== null && ressetpass === usernameValue) {
          //   setShowSuccessAlert(true)
          //   setSuccessAlertMessage('Password Set Successfully')
          //   setTimeout(() => {
          //     setShowSuccessAlert(false)
          ...
          if (setpass && setpass.MessageTypeID === 3) {
            setShowSuccessAlert(true)
            setSuccessAlertMessage('Password added successfully, Please login again!')
            setTimeout(() => {
              setShowSuccessAlert(false)
              localStorage.clear()
              router.push('/login/form')
            ...
            <div className="col-12 col-sm-12 col-md-12 col-lg-12">
              <div className="section-title text-center">
                <p align="center" style={{ padding: '10px,0px' }}>
                  <Link href="/">
                    {isClient ? <img style={{ paddingLeft: '23px' }} referrerPolicy='no-referrer' src={`${logoURL ? logoURL : '/assets1/img/icon/adibuja-logo.svg'}`} width="198" height="42" alt={"brand-logo"} objectPosition={'center'} /> : ''}
                  </Link>
                </p>
              </div>
            </div>

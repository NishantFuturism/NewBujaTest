...
const TermsAndConditions = ({ termsAndConditions, sellerIDFromServer, sellerDetailsFromDomain }) => {
  const [Ftr, setFooter] = useState(termsAndConditions);
  ...
  return (
    <>
      <Head>
        <title>{termsAndConditions?.PageTitle}</title>
        <meta name="description" content={termsAndConditions?.MetaDescription} />
        <meta name="keywords" content={termsAndConditions?.MetaKeyword}></meta>
        <meta property="og:title" content={termsAndConditions?.PageTitle} />
        <meta property="og:url" content={`/${termsAndConditions?.PageUrl}`} />
        <meta property="og:description" content={termsAndConditions?.MetaDescription}/>
      </Head>
...
function getTermsAndConditionsData(sellerIdMain) {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.footer}pagename=terms-and-conditions&cid=1&languageId=2&sellerid=${sellerIdMain}`;

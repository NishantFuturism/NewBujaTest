...
export function RecommendedProduct() {
  ...
  const CustomButtonGroupAsArrows = ({ next, previous, onClick, ...rest }) => {
    ...
    return (
      ...
      items-center w-full">
        <button type="button" className='previous round1' aria-label="previousbtn" onClick={() => previous()}>
          <FiChevronLeft />
          {/* )} */}
        </button>
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && window.localStorage && localStorage.getItem('CustGUID')) {
      ProductDetailAPI.getrecomended()
      .then(response => {
        
        setgetDealofday(response && response.skuListingModels ? response.skuListingModels : [])
  ...
  return (
    <>
      {homeScreen && homeScreen.loading !== true && Dealofday !== null && Dealofday !== undefined ?
        <div className="container-fluid homescreenrecommended" style={{display: Dealofday.length === 0 ? 'none' : 'block'}}>
          {console.log('Dealofday',Dealofday)}
          <div className="row" >
            <div className="col-12" >
              ...
              {Dealofday && Dealofday.length === 0 ? null :
                <Carousel breakPoints={responsive} itemsToShow={4} itemsToScroll={1} pagination={false} >
                  {Dealofday?.length > 0 && (Dealofday.slice(sliceNum, sliceNum + Dealofday.length) || []).map((mapdata, i) => (
                    mapdata.FiltersList.length !== 0 ?
                      <div className="tab-content" key={sliceNum} style={{ margin: "15px" }}>
                        <div className="tab-pane fade show active" id="brand-one" style={{ display: 'block' }} >

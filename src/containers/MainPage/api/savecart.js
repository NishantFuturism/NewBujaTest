...
function savecartlisting() {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.savcartlisting}custGUID=${(localStorage.getItem('CustGUID') === null || localStorage.getItem('CustGUID') === undefined) ? ConstantsValues.defaultCustGUID : window.atob(localStorage.getItem('CustGUID'))}&CurrencyCode=${ConstantsValues.currencyCode}&pincode=${Cookies.get('pincodevalue') || localStorage.getItem('pincodevalue') || ConstantsValues.pincode}`;
...
// }
function savecartItemlisting(listname) {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.viewsavecart}custGuid=${(localStorage.getItem('CustGUID') === null || localStorage.getItem('CustGUID') === undefined) ? ConstantsValues.defaultCustGUID : window.atob(localStorage.getItem('CustGUID'))}&cartId=${(localStorage.getItem('CartGUID') === null || localStorage.getItem('CartGUID') === undefined) ? ConstantsValues.defaultCartGUID : window.atob(localStorage.getItem('CartGUID'))}&addressid=1&clientId=${ConstantsValues.ClientId}&listName=${listname}&pinCode=${Cookies.get('pincodevalue') || localStorage.getItem('pincodevalue') || ConstantsValues.pincode}&currencyName=${ConstantsValues.currencySymbol}&currencySymbol=${ConstantsValues.currencyCode}&isUserAuthenticated=${!((localStorage.getItem('CustGUID') === null || localStorage.getItem('CustGUID') === undefined))}`;
  return httpRequest(url, 'GET',);
}

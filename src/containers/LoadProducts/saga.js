...
import { ADD_TO_CART, ADD_TO_CART_SUCCESS, ADD_TO_COMPARE, ADD_TO_WISHLIST, ADD_TO_WISHLIST_SUCCESS, REMOVE_FROM_CART, RESPONCE_MSG, SHOPPING_CART_DETAILS, PRODUCT_COMPARE_SUCCESS, RECENT, RECENT_SUCCESS, QUICKVIEW, QUICKVIEW_SUCCESS,GET_TO_COMPARE,RESPONCE_COMPARE } from './constants';
...
function* GetCompareAPI(action) {
    ...
    const newUpdateCompare = yield call(CustomsAPI.getnewcompare, action.SkuId, action.ip, action.compareflag)
    const compareSkcode =  yield call(CustomsAPI.getproductcompare) 
    console.log('compareSkcode',compareSkcode)
    console.log('newUpdateCompare',newUpdateCompare)

    toast(newUpdateCompare)
    
    yield put({ type: RESPONCE_COMPARE, compareSkcode });   
}
...
export default function* loadProductsSaga() {
  ...
  yield takeEvery(GET_TO_COMPARE, GetCompareAPI);

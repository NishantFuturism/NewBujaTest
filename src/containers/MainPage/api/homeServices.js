...
function getMegamainmenu() {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.MegaMainMenu}ClientId=${ClientId}&languageId=${languageId}&IsForReact=1&sellerid=${localStorage.getItem('sellerID') || ConstantsValues.defaultSellerID}`;
  return httpRequest(url, 'GET');
}
function getMultiMenu() {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.MultiMenu}ClientId=${ClientId}&languageId=${languageId}&IsForReact=1&sellerid=${localStorage.getItem('sellerID') || ConstantsValues.defaultSellerID}`;
  return httpRequest(url, 'GET');
}
...
// }
function getBanners() {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.banners}ClientId=${ClientId}&sellerid=${localStorage.getItem('sellerID') || ConstantsValues.defaultSellerID}`;
  return httpRequest(url, 'GET');
}
...
function getShopbycategory() {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.ShopByCategory}ClientId=${ClientId}&sellerid=${localStorage.getItem('sellerID') || ConstantsValues.defaultSellerID}`;
  return httpRequest(url, 'GET');
}
...
function getBrandList(PageNumber, PageSize) {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.BrandList}ClientId=${ClientId}&IsgetAll=${IsgetAll}&displayAllBrands=${1}&PageNumber=${PageNumber}&PageSize=${PageSize}&sortby${'POP'}&sellerid=${localStorage.getItem('sellerID') || ConstantsValues.defaultSellerID}`;
...
function getFooter(Description) {
  ...
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.footer}pagename=${pagename}&cid=${ClientId}&languageId=${languageId}&sellerid=${localStorage.getItem('sellerID') || ConstantsValues.defaultSellerID}`;
  return httpRequest(url, 'GET');
}
...
function geTopsellingai() {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.topsellingai}languageId=${languageId}&IsgetAll=${IsgetAll}&SkuDetailid=${0}`;
  return httpRequest(url, 'GET');
}
...
function GetCustIdByEmail(username) {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.GetCustIdByEmail}clientid=${ClientId}&email=${username}&sellerid=${localStorage.getItem('sellerID') || ConstantsValues.defaultSellerID}`;
  return httpRequest(url, 'POST');
}
function getCustIdUsingGmailId(clientid, emailId, customerType) {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.GetCustIdByEmail}clientid=${clientid}&email=${emailId}&customerType=${customerType}&sellerid=${localStorage.getItem('sellerID') || ConstantsValues.defaultSellerID}`;
  return httpRequest(url, 'POST');
}
...
function generateotpforregistrationnew(username) {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.generateotpforregistrationnew}userName=${username}&sellerId=${localStorage.getItem('sellerID') || defaultSellerID}`;
  return httpRequest(url, 'POST');
}
...
function removewishlist(SkuId, WishlistGUID, SKUFilterPriceId) {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.wishlistremoveitem}customerGUID=${(localStorage.getItem('CustGUID') === null || localStorage.getItem('CustGUID') === undefined) ? defaultCustGUID : window.atob(localStorage.getItem('CustGUID'))}&wishlistGUID=${WishlistGUID}&skuId=${SkuId}&skufilterPriceId=${SKUFilterPriceId}&sellerid=${localStorage.getItem('sellerID') || ConstantsValues.defaultSellerID}`;
  return httpRequest(url, 'POST');
}
function sendOtpLoginApi(userID) {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.sendOtpLogin}userName=${userID}&customerType=${'1'}&sellerId=${localStorage.getItem('sellerID') === null || localStorage.getItem('sellerID') === undefined ? ConstantsValues.defaultSellerID : localStorage.getItem('sellerID')}`
  return httpRequest(url, 'POST');
}
...
function sendOtpRegistrationApi(userID) {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.generateotpforregistrationnew}userName=${userID}&sellerId=${localStorage.getItem('sellerID') || defaultSellerID}`
  return httpRequest(url, 'POST');
}
...
function forgotpasswordApi(id) {
  ...
  const CType = 1
  const ClId = 1
  const obj = { UserName, CType, ClId, SellerId }
...
function verifyOTP(UserName, OTP) {
  const SellerId = localStorage.getItem('sellerID') === null || localStorage.getItem('sellerID') === undefined ? ConstantsValues.defaultSellerID : localStorage.getItem('sellerID')
  const obj = { UserName, OTP, SellerId }
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.verifyLoginOTP}`
  return httpRequest(url, 'POST', obj);
}
function verifyRegistrationOTP(customerRegistrationOTPGUID, OTP) {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.verifyregistrationotp}customerRegistrationOTPGUID=${customerRegistrationOTPGUID}&otp=${OTP}&clientid=${ConstantsValues.ClientId}&sellerId=${localStorage.getItem('sellerID') || defaultSellerID}`
  return httpRequest(url, 'POST');
}
...
function verifyPassword(UserName, Password) {
  ...
  const obj = { UserName, Password, CustomerType, SellerId }
...
function getproductcompare() {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.skusfromcodes}cartId=${(localStorage.getItem('CartGUID') === null || localStorage.getItem('CartGUID') === undefined) ? ConstantsValues.defaultCartGUID : window.atob(localStorage.getItem('CartGUID'))}&custGuid=${(localStorage.getItem('CustGUID') === null || localStorage.getItem('CustGUID') === undefined) ? ConstantsValues.defaultCustGUID : window.atob(localStorage.getItem('CustGUID'))}&CurrencyCode=INR&sellerid=${localStorage.getItem('sellerID') || ConstantsValues.defaultSellerID}`;
...
function getnewcompare(SkuId,ip,compareflag) {
  // const flag=false
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.addtocompare}custguid=${(localStorage.getItem('CustGUID') === null || localStorage.getItem('CustGUID') === undefined) ? ConstantsValues.defaultCustGUID : window.atob(localStorage.getItem('CustGUID'))}&skudetailid=${SkuId}&sessionid=${SessionId}&ipAddress=${ip}&flag=${compareflag}&sellerid=${localStorage.getItem('sellerID') || ConstantsValues.defaultSellerID}`;
  return httpRequest(url, 'POST',); 
}
...
function getTopTrendingProductList(pageNum, PageSize, SkuDetailId) {
  if (typeof window !== 'undefined' && window.localStorage) {
    const productcategory = localStorage.getItem('ProductName') !== "" ? localStorage.getItem('ProductName') : ""
    const url = `${Constants.urls.baseUrl}${Constants.endPoints.GetTopTrendingProductList}pincode=${ConstantsValues.pincode}&LanguageId=${ConstantsValues.languageId}&CartGUID=${(localStorage.getItem('CartGUID') === null || localStorage.getItem('CartGUID') === undefined) ? ConstantsValues.defaultCartGUID : window.atob(localStorage.getItem('CartGUID'))}&CustGUID=${(localStorage.getItem('CustGUID') === null || localStorage.getItem('CustGUID') === undefined) ? ConstantsValues.defaultCustGUID : window.atob(localStorage.getItem('CustGUID'))}&clientId=${ConstantsValues.ClientId}&currencyCode=${currencyCode}&IsgetAll=${IsgetAll}&PageNumber=${pageNum}&PageSize=${PageSize}&skudetailid=${SkuDetailId || 0}&sellerid=${localStorage.getItem('sellerID') || ConstantsValues.defaultSellerID}`;
    return httpRequest(url, 'GET');

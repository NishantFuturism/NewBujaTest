...
const Careers = ({ careersData, sellerDetailsFromDomain }) => {
  const [Ftr, setFooter] = useState(careersData);
  ...
  return (
    <>
      <Head>
        <title>{careersData?.PageTitle}</title>
        <meta name="description" content={careersData?.MetaDescription} />
        <meta name="keywords" content={careersData?.MetaKeyword}></meta>
        <meta property="og:title" content={careersData?.PageTitle} />
        <meta property="og:url" content={`/${careersData?.PageUrl}`} />
        <meta property="og:description" content={careersData?.MetaDescription}/>
      </Head>
...
function getCareersDataData() {
  const url = `${Constants.urls.baseUrl}${Constants.endPoints.footer}pagename=careers&cid=1&languageId=2`;

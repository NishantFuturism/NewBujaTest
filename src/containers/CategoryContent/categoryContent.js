    ...
    <div>
      ...
      <section className="categoryholder">
        <div className="container-fluid pt-20 pb-20">
          ...
          {groupedData && groupedData.map((arr, i) => (
            <div className='catonMobile' key={i}>
              <h3>{arr[0]}</h3>
              <div className="row catboxcate">
                {arr[1].map((itm, i) => (
                  <div key={i} className="col-lg-2 col-sm-6 col-xs-6 col-6">
                    <div>
                      <Link href={itm.PageUrl}>
                        {/* <img referrerPolicy='no-referrer' src={itm.CategoryImage} alt={itm.ImageAlt} style={{ width: '300px' }} onClick={() => productlistpage(itm.PageUrl)} /> */}
                        <Image 

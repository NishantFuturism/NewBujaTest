...
import homeServicesAPI, { ConstantsValues } from "../containers/MainPage/api/homeServices";
...
export function RegisterOrignal() {
  ...
  const [showSendSuccessfullyMsgCount, setShowSendSuccessfullyMsgCount] = useState(0)
  ...
  const [isRegisterClicked, setIsRegistereClicked] = useState(false);
  ...
  if (typeof window !== 'undefined' && window.localStorage) {
    if (localStorage.getItem("UserMobileEmailFromLogin")) {
      setUserName(localStorage.getItem("UserMobileEmailFromLogin"))
      localStorage.setItem('UserMobileEmailFromLogin', '')
    }
  }
  }, [])
  useEffect(() => {
  if (typeof window !== 'undefined' && window.localStorage) {
    if (localStorage.getItem("rediractFromLogin") === "/login") {
      ...
      setTimeout(()=>{
        localStorage.setItem('rediractFromLogin', '')
      },1000)
    }
  }
  }, [userName])
  ...
  useEffect(() => {
    ...
    if (userName !== '' && registerOrignal !== undefined && registerOrignal.sendOTPData !== undefined && registerOrignal.sendOTPData.IsCustomerExist === true) {
      ...
      clearTimeout()
      setShowOTPView(false)
      localStorage.setItem('UserMobileEmailFromRegister', userName)
      localStorage.setItem('rediractFromRegister', "/register")
      setTimeout(() => {
        router.push({ pathname: '/login'})
        // document.getElementById('sendloginUserWithOTP').click()
      }, 2000)
    }
    if (showSendSuccessfullyMsgCount < 1 && !isRegisterClicked && userName !== '' && registerOrignal !== undefined && registerOrignal.sendOTPData !== undefined && registerOrignal.sendOTPData.Message === 'OTP sent successfully' && registerOrignal.sendOTPData.IsCustomerExist === false) {	
      setIsError(false)	
      setMsg('OTP sent successfully')	
      setShowMsg(true)	
      closeMsgBar();	
      setShowSendSuccessfullyMsgCount(0)	
    }
...
//      console.log('registerOrignal.OTPVerify ---', registerOrignal, userName)
  ...
  async function Regesternow(e) {
    ...
    if (enteredOTP === '') {
      ...
      const isOTPVerified = await CustomsAPI.verifyRegistrationOTP(registerOrignal.sendOTPData.CustomerGUID, enteredOTP);
  ...
  //   const input = document.getElementById('txtRegisterOTP');
  //   // console.log('i100',input.value)
  //   const text = input?.value;
  //   const lastLetter = text.slice(-1); // Get the last letter
  //   input.setAttribute('data-last-letter', lastLetter); // Set it as an attribute
  //   }
  // },[])
  ...
  function sendOtpFunction() {
    ...
    if (document.getElementById('txtRegisterUserNameForOTP').value === '') {
      ...
      setShowOTPView(false)
      setTimeout(() => {
        router.push({ pathname: '/login', query: { data: userName, rediractFrom: "/register" }})
        // document.getElementById('sendloginUserWithOTP').click()
      }, 2000)
    } 
    else {
      ...
      // console.log("otpsection",document.getElementById('txtRegisterOTP').value)
      // console.log("otpsection2",document.getElementById('txtRegisterUserNameForOTP').value);
      dispatch(sendOtp(document.getElementById('txtRegisterUserNameForOTP').value))
    }
  }
                                      ...
                                      <div className="login-form" id="divRegisterOTPForm" style={{ padding: '25px' }}>
                                        ...
                                        {enablechangeview === false ?
                                          <div className="form-group row align-items-center mb-4 reg_Otp_rw">
                                            <div className="rgt_ipt_box">
                                              ...
                                              <p>
                                                <Link id="registerUserWithOTP" href="" onClick={(e) => sendOTPButtonClick(e)}
                                        ...
                                        {ShowOTPView && (
                                          <div className="login-box text-center" id="divOTPsection" >
                                            ...
                                            <button type="button" id="btnRegisterWithOTP" className="btn btn-secondary mb-4 mt-4" onClick={(e) => Regesternow(e)}>
                                              Register Now
                                            </button>
                                            {UpdatedTimer === 0 ?

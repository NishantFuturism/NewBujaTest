...
export function OrderPlacemode(props) {
  ...
  const {setPaymentInfoSuccessPage} = useContext(Message_data);
  ...
  useEffect(()=> {
    if (typeof window !== 'undefined' && window.localStorage) {
      if (!localStorage.getItem('CustGUID')) {
        router.push('/')
      }
      let custGUID = localStorage.getItem('CustGUID');
      if (custGUID && custGUID==='00000000-0000-0000-0000-000000000000') {
        router.push('/')
      }
    }
  })
  ...
  useEffect(() => {
    mycartAPI.getCartCommon()
      .then(response => {
        ...
        if (response && response.CartGuid && response.CartGuid!=="") {
          localStorage.setItem('CartGUID', window.btoa(response.CartGuid))
        } else {
          ...
          mycartAPI.getCartnew()
            ...
            .then(response => {
              ...
              if (response[0].Total === 30) {
                ...
                console.log("cartReducer==response==", buyamount, paymentmode, response[0].Total);
  ...
  const loadRazorpay = () => {
    ...
    console.log("RemAmount===", buyamount, amount, RemAmount);
    console.log("updatedelivery time===", DeliverySlotDate, DeliverySlotID, InstructionForDeliverySlot);
  ...
  function placeorder(newdata, recivedata, response, respo) {
    console.log("placeorder",{ paymentType, CustomerAddressId, recivedata });
    ...
    PaymentAPI.shiprocketCreateOrder(localStorage.getItem('transactionid'), paymentType, CustomerAddressId, paymentmethod)
    .then(orderres => {
        console.log('shiprocketCreateOrder', orderres)
        let shipOrderId = orderres.split(':')[1].trim();
        //if (shipOrderId) {
          localStorage.setItem('OrderNumber', window.btoa(shipOrderId))
          PaymentAPI.captutreRazorpay(databody)
            .then(captuure => {
             console.log('captuure', captuure)
              if (captuure === 'success') {
                PaymentAPI.razorpayVerifysignature(databody)
                  .then(verify => {
                    ...
                    if (verify) {
                      ...
                      // setloading(true)
                      PaymentAPI.updateDeliveryslotTime(shipOrderId, DeliverySlotDate, DeliverySlotID, InstructionForDeliverySlot)
                      setPaymentInfoSuccessPage({amount, paymentmethod})
    ...
    PaymentAPI.initiatetransaction(paymentmode)
      .then(response => {
        if (response) {
          ...
          PaymentAPI.shiprocketCreateOrder(response, paymentType, CustomerAddressId, paymentmethod)
          .then(res => {
              console.log("shiprocketCreateOrder",res);
              let shipOrderId = res.split(':')[1].trim();
              console.log("shiprocketCreateOrder",shipOrderId);
              ...
              //if (shipOrderId) {
                ...
                //PaymentAPI.updateDeliveryslotTime(res.OrderId, DeliverySlotDate, DeliverySlotID, InstructionForDeliverySlot)
                PaymentAPI.updateDeliveryslotTime(shipOrderId, DeliverySlotDate, DeliverySlotID, InstructionForDeliverySlot)
                setPaymentInfoSuccessPage({amount, paymentmethod})
                setloading(true)
                router.push('/product/checkout/order/paymentsuccess',)
  ...
  useEffect(() => {
    ...
    if(walletbalance === 0 && amount > 5000){
      console.log("594 walletbalance=====")
      setWarnstatus(true)
        setCashDeliverymsg(`Cash on Delivery is applicable only for bills up to Rs. 5000/-`)
        setTimeout(() => {
          setWarnstatus(false)
  ...
  return (
    <>
      ...
      <div className="login-wrapper pb-35 proceedMainContainer">
        <div className="container-fluid">
          <div className="row">
            <div className="col-12 col-sm-12 col-md-12 col-lg-12">
              <main className="site-main" id="primary">
                <div className="user-login proceedtobuyStatusbar">
                  <div className="row">
                                          ...
                                          <div id="payment_stage" className="checkout-form">
                                            ...
                                            <div className="col-lg-12 checkoutPadding" >
                                              <div>
                                                ...
                                                >
                                                  ...
                                                  <div className="custom-radio cod_radio" style={{ opacity: payonline ? 0.2 : 1 }}>
                                                  ...
                                                  <div className="custom-radio cod_radio" style={{ opacity:!cashdelivery ? 1 : 0.2 }}>
                                                    <label className="paymentoptionctr" >
                                                      <b>Cash On Delivery</b>
                                                      <input type="radio"
                                                        checked={codcheck}
                                                        //disabled={!cashdelivery}
                                            ...
                                            <div className="col-lg-12 amt_btn_rW checkoutAmount">
                                              ...
                                              <div className="order-lg-last w-50">
                                                <input
                                                  disabled={paymentButton && paymentButton===true ? false : true}

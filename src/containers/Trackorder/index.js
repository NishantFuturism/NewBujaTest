...
export function Trackorder() {
  ...
  useEffect(() => {
    ...
    //   setorderstatus(4)
    // }
  }, [(window.atob(localStorage.getItem('OrderStatus')))])
  ...
  return (
    <>
      ...
      <div className="container-fluid trackStatusbar">
        <h4> <b>Order Number :{window.atob(localStorage.getItem('OrderNumber'))}</b></h4>
        {/* <div>
        ...
        }> <Link to={`/account/myorders/myorder-detail/${window.atob(localStorage.getItem('OrderNumber'))}`} className="btn btn-secondary order-btn">← Back to My Order </Link> </div >
      </div >
      <Footer />
    </>
  );

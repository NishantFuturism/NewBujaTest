...
export default function ShopBrand(props) {
  ...
  const [Dealofday, setgetDealofday] = useState(props.brandsDataHomePage && props.brandsDataHomePage.Data || []);
  ...
  const CustomButtonGroupAsArrows = ({ next, previous, onClick, ...rest }) => {
    ...
    return (
      ...
      items-center w-full">
        <button type="button" className='previous round1' id='prevRound1' aria-label="previousbtn" onClick={() => previous()}>
          <FiChevronLeft />
          {/* )} */}
        </button>
    ...
    <div id='brandsSlider1'>
      ...
      >
        ...
        {Dealofday !== undefined && (Dealofday.slice(sliceNum, sliceNum + Dealofday.length) || []).map((mapdata, i) => (
          ...
          //   </div> */}
          // </>
          <React.Fragment key={`shopbrand ${i}`}>
            {
              mapdata.Thumbnail ? <>
                <div style={{ marginRight: "5px", height: "80px" }}><Link href={`/brands/${mapdata.PageUrl}`}>
                  <span style={{ margin: '0px 10px 0px 10px' }}>
                    {/* <img className="" src={mapdata.Thumbnail} referrerPolicy='no-referrer' alt="britannia" onClick={() => brandProduct(mapdata.PageUrl)} /> */}
                    <Image 
                      className="" 
      ...
      <div className="row">
        <div className="col-12">
          <div className="container-fluid pt-60 pb-20 homescreenshopbybrand">
            <div>
              {Dealofday !== undefined ?
                <><div style={{ float: "left" }}><h3>Shop by Brands</h3></div><div className="brandViewall" style={{ paddingTop: "10px", float: "right" }}><h6 style={{ color: "black", fontWeight: "600" }}><a href='/allbrands'>View All</a></h6></div>
